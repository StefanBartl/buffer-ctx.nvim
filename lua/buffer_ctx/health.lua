...
function M.check()
  ...
  -- Cross-plugin shims on :Insert/:Copy (commands.lua's "mdlink"/
  ...
  if pcall(require, "markdown.commands.markdown_links") then
    vim.health.ok(
      "markdown.nvim detected — :Insert/:Copy mdlink delegates to its own link builder (optional dependency)"
    )
  else
    vim.health.info(
      "markdown.nvim not found — :Insert/:Copy mdlink falls back to a literal [title](path) (optional dependency)"
    )
  end
  ...
  if vim.fn.exists(":CopyFilepathRepos") == 2 then
    vim.health.ok(":CopyFilepathRepos compat command registered")
  else
  ...
  if cfg_ok and type(cfg_mod.issues) == "function" then
    ...
    if #issues > 0 then
      ...
      vim.health.ok("config: no unknown or invalid options from the last setup() call")
    end
  end
